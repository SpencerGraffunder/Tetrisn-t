...
class ClientGame(States):
    ...
    def update(self, dt):
        ...
        g.tile_size = (4 * g.player_count) + 6

        g.tile_size = min(WINDOW_WIDTH,WINDOW_HEIGHT) // max(self.state.board_width,g.BOARD_HEIGHT)
    ...
    def draw(self, screen):
        ...
        for row_index, tile_row in enumerate(self.state.board[2:]):
            for col_index, tile in enumerate(tile_row):
                ...
                col_index * g.tile_size + centering_offset,
                row_index * g.tile_size))
        ...
        (Constants.WINDOW_WIDTH) - (4 * g.tile_size),
        (g.BOARD_HEIGHT - 3) * g.tile_size), (0, 128, 0))

        score_val = '%d' % (self.state.score)
        text.draw(screen, score_val, 'SMALL', (
        (Constants.WINDOW_WIDTH) - (4 * g.tile_size),
        (g.BOARD_HEIGHT - 2) * g.tile_size), (0, 128, 0))

        # display level
        level_str = 'Level: %d' % (self.state.current_level)
        text.draw(screen, level_str, 'SMALL', (
        (Constants.WINDOW_WIDTH) - (4 * g.tile_size),
        (g.BOARD_HEIGHT - 4) * g.tile_size), (0, 128, 0))
    ...
    def draw_next_piece(self, screen, player_number):
        ...
        for tile in player.next_piece.locations:
            scaled_image = pygame.transform.scale(
                ...
                (g.tile_size, g.tile_size))
            screen.blit(scaled_image,
                        (x_offset+((tile[0]-player.spawn_column)*g.tile_size),
                         (tile[1])*g.tile_size+y_offset))